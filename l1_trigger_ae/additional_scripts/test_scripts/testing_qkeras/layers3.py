...
qbits = quantized_relu(4, 0, use_ste=True)
qbits_norelu = quantized_relu(4, 0, 1, use_ste=True, linear= True)
# Crear modelo con varias capas QKeras
model = Sequential([
    QConv2D(16, (3, 3), strides=(2, 2), padding="same",
            kernel_quantizer=qbits, bias_quantizer=qbits_norelu,
            input_shape=(16, 16, 3)),
    QActivation(qbits),  # Usamos qbits en la activación para mantener la cuantización
    MaxPooling2D(),
    ...
    QActivation(qbits),
    QDense(10, kernel_quantizer=qbits_norelu, bias_quantizer=qbits)
])
