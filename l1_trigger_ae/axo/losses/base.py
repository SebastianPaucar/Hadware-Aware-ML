...
class L1ADBaseLoss(Loss):
    def __init__(self, norm_scales, norm_biases, mask, unscale_energy = False,name="L1ADLOSS"):
        ...
        PT_SCALER: np.ndarray = np.array([PT_CALO_SCALER] * 13 + [PT_MUON_SCALER] * 8 + [PT_CALO_SCALER] * 24) #12 without taus
        ETA_SCALER: np.ndarray = np.array([CALO_ETA_SCALER] * 13 + [MUON_ETA_SCALER] * 8 + [CALO_PHI_SCALER] * 24) #12 without taus
        PHI_SCALER: np.ndarray = np.array([CALO_PHI_SCALER] * 13 + [MUON_PHI_SCALER] * 8 + [CALO_PHI_SCALER] * 24) #12 without taus
