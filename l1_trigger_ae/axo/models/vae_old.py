    ...
    if QAT_config:
        ...
        for i,node in enumerate(encoder_config["nodes"]):
                x = QDense(node,
                           ...
                           kernel_quantizer=quantized_relu(*ap_fixed_kernel,linear=True,use_ste=True),
                           bias_quantizer=quantized_relu(*ap_fixed_bias,linear=True,use_ste=True),
                           kernel_initializer="glorot_uniform",
                           bias_initializer="zero",
                           activation = quantized_relu(*ap_fixed_activation,use_ste=True)
        ...
        z_mean = QDense(latent_dim,
                               ...
                               kernel_quantizer=quantized_relu(*ap_fixed_kernel,linear=True,use_ste=True),
                               bias_quantizer=quantized_relu(*ap_fixed_bias,linear=True,use_ste=True),
                               activation=quantized_relu(*ap_fixed_activation,linear=True,use_ste=True)
                               # kernel_regularizer=regularizers.l1(0) ### commented out after Chang's finding ...
                              )(x)
...
class VariationalAutoEncoderHGQ(Model):
    ...
    def train_step(self, data):
        ...
        grad_dict = {id(var): grad for var, grad in zip(self.trainable_weights, grads)}
        for layer in self.encoder.layers:
            trainable_ids = set(id(v) for v in layer.trainable_weights)
            for w in layer.weights:
                is_trainable = id(w) in trainable_ids
                grad = grad_dict.get(id(w), None)
                if grad is None:
                    grad_str = "None"
                else:
                    grad_str = f"mean(abs): {tf.reduce_mean(tf.abs(grad)).numpy():.6f}"
                print(f"Layer: {layer.name:<20} | Weight: {w.name:<35} | Shape: {str(w.shape):<15} | Trainable: {is_trainable} | Grad: {grad_str}")
...
class PrintLatentLogVarWeights(tf.keras.callbacks.Callback):

    def on_epoch_begin(self, epoch, logs=None):
        log_var_layer = self.model.encoder.get_layer('latent_log_var')
        kernel_weights_log_var, bias_weights_log_var = log_var_layer.get_weights()
        mean_layer = self.model.encoder.get_layer('latent_mean')
        kernel_weights_mean, bias_weights_mean = mean_layer.get_weights()

        print(f"\n[Epoch {epoch+1} BEGIN] latent_mean kernel (sample): {kernel_weights_mean.flatten()[:20]}")
        print(f"[Epoch {epoch+1} BEGIN] latent_mean bias (sample): {bias_weights_mean.flatten()[:20]}")

        print(f"\n[Epoch {epoch+1} BEGIN] latent_log_var kernel (sample): {kernel_weights_log_var.flatten()[:20]}")
        print(f"[Epoch {epoch+1} BEGIN] latent_log_var bias (sample): {bias_weights_log_var.flatten()[:20]}")
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(f"\n[Epoch {epoch+1} END] latent_mean kernel (sample): {kernel_weights_mean.flatten()[:20]}")
        print(f"[Epoch {epoch+1} END] latent_mean bias (sample): {bias_weights_mean.flatten()[:20]}")

        print(f"\n[Epoch {epoch+1} END] latent_log_var kernel (sample): {kernel_weights_log_var.flatten()[:20]}")
        print(f"[Epoch {epoch+1} END] latent_log_var bias (sample): {bias_weights_log_var.flatten()[:20]}")
