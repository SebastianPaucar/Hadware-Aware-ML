...
def store_axo(config,model,model_trim,axo_man,dist_plot,history_dict):
  ...
  #  if not skip_flag:
   #     c_latent = c_axo.create_group("latent_scores")
   # l_latent = l_axo.create_group("latent_scores")

    #latent_scores = axo_man.get_latent_scores()

  #  max_len = max(len(scores) for scores in latent_scores.values())
  #  for signal_name in latent_scores.keys():
  #      scores = latent_scores[signal_name]
  #      if len(scores) < max_len:
  #          padded = np.full(max_len, np.nan, dtype=np.float32)
  #          padded[:len(scores)] = scores
  #          latent_scores[signal_name] = padded
  #      else:
  #          latent_scores[signal_name] = np.array(scores, dtype=np.float32)

   # for signal_name, scores in latent_scores.items(): # This includes the background as a key
   #     scores_np = np.array(scores, dtype=np.float32)
   #     if not skip_flag:
   #         c_latent.create_dataset(name=signal_name, data=scores_np)
   #     l_latent.create_dataset(name=signal_name, data=scores_np)
    ...
    #LATENT SCORES STORAGE (independent group)
    if not skip_flag:
        c_latent = c_results.create_group("latent_scores")
    l_latent = l_results.create_group("latent_scores")

    latent_scores = axo_man.get_latent_scores()
    for signal_name, scores in latent_scores.items():  # Each signal/background can have different length
        scores_np = np.array(scores, dtype=np.float32)
        if not skip_flag:
            c_latent.create_dataset(name=signal_name, data=scores_np)
        l_latent.create_dataset(name=signal_name, data=scores_np)
